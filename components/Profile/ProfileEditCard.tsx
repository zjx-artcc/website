...
export default function ProfileEditCard({
  ...
  const handleSubmit = async (formData: FormData) => {
    const User = z.object({
      preferredName: z
        .string()
        .max(40, "Preferred name must not be over 40 characters")
        .optional(),
      bio: z
        .string()
        .max(400, "Bio must not be over 400 characters")
        .optional(),
      operatingInitials: z
        .string()
        .length(2, "Operating Initials must be 2 characters")
        .toUpperCase(),
    ...
    const result = User.safeParse({
      ...
      operatingInitials:
        (formData.get("operatingInitials") as string) || user.operatingInitials,
      receiveEmail: true,
      newEventNotifications: formData.get("newEventNotifications") === "on",
    });

    if (!result.success) {
      toast(result.error.errors.map((e) => e.message).join(".  "), {
        type: "error",
      });
      return;
    }
    ...
    if (
      filter.isProfane(result.data.preferredName || "") ||
      filter.isProfane(result.data.bio || "")
    ) {
      await writeDossier(
        admin
      ...
      toast(
        "Please ensure your preferred name and bio do not contain any profanity.  This incident has been added to your dossier.",
        { type: "error" }
      );
      return;
    }
          ...
          <Stack direction="column" spacing={2}>
            ...
            {admin && (
              <TextField
                ...
                name="discordId"
                label="Discord ID"
								helperText="This field is used to fix problems with accounts being linked incorrectly."
